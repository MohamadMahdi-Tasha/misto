...
export default function PageCategoriesFilterHolderComponent(props) {
    ...
    function handleClickOfFilterToggleBtn() {
        setTimesClickedOnFilterToggler(prevState => prevState + 1);

        (timesClickedOnFilterToggler % 2 !== 0)
            ? setFilterOpenedOrClosed(true)
            : setFilterOpenedOrClosed(false)
    }
    ...
    return (
        <div className='page__categories-filter-holder'>
            <div className='page__categories-filter-top-side'>
                <button data-opened={isFilterOpened} onClick={handleClickOfFilterToggleBtn} className='page__categories-filter-btn'>
